...
# Plots rendered via these tags use include_plotlyjs='cdn' rather than the plotly.offline.plot
# default of True (which embeds the full ~3-4MB Plotly.js library inline in every plot's HTML).
# On the AWS deployment, those large embedded scripts were getting truncated somewhere in the
# delivery path, leaving 'Plotly is not defined' errors in the browser console and no plots at
# all. These MOP-owned copies of tom_targets'/tom_dataproducts' own templatetags exist solely to
# fix that one keyword argument; the rest of the logic mirrors upstream exactly.

@register.inclusion_tag('tom_targets/partials/target_plan.html', takes_context=True)
def mop_target_plan(context, fast_render=False, width=600, height=400, background=None, label_color=None, grid=True):
    """
    Mirrors tom_targets.templatetags.targets_extras.target_plan, with include_plotlyjs='cdn'.
    """
...
def mop_moon_distance(target, day_range=30, width=600, height=400, background=None, label_color=None, grid=True):
    """
    Mirrors tom_targets.templatetags.targets_extras.moon_distance, with include_plotlyjs='cdn'.
    """
...
def mop_spectroscopy_for_target(context, target, dataproduct=None):
    """
...
def mop_photometry(mulens):
    ...
    return {
        'target': mulens,
        'plot': offline.plot(fig, output_type='div', show_link=False, include_plotlyjs='cdn')
    }
